...
<main class="rp-dashboard">
    ...
    if ($tab === 'start') {
        ...
        $whereConditions[] = "status = 'Active'"; // Manual Trigger check
        $whereConditions[] = "beneficiaries > (SELECT COUNT(id) FROM distribution_logs WHERE program_id = aid_program.id)"; // Capacity Trigger check
    } 
    elseif ($tab === 'locked') {
        $whereConditions[] = "date_scheduled > '$today'";
    } 
